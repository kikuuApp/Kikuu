const English={
    /**Telephone */
    reg_instruction:'Enter a personal fully working'
                    +' mobile number in international format.'
                    +'\nExample [+44 123 456 7890].'
                    +'\n Mobile sms tariff my apply.',
    reg_input_placeholder: '+44 - 123 - 456 - 7890',
    reg_continue:'Continue',
    reg_errorMsg: 'Mobile phone number must start with country code,'+
                  'followed by the number without the leading zero (0) in the front',
    reg_authcode_placeholder: '#####',              
    
    /**Pincode */
    reg_pincodeMsg:'pinCode match error!',
    reg_pincode_RequestMsg:'You will shortly recieve authorization pinCode.',
    reg_pincode_do: 'Enter recieved sms authorization code in the space provided',
    /***System Navigation */
    activities: 'Activities',
    contacts: 'Contacts',
    
};
export default English;